...
Client to Server requests are standard HTTP requests. \\
POST will be used for sensitive data such as "Username" and "Password" strings
to avoid leaving personally identifiable information behind in places
such as the browser's url history.\\
\\
GET requests with parameters will be used in the "floor" and "room" lookup
requests. \\

Server to Data Store communications will be handled trough standards SQL queries.
...
be used. As DjAngo and database driver itself abstracts the details away from 
our implementation no further details will be discussed. \\
